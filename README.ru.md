...
1. **Счётчик повторений** — отслеживает, как часто факт упоминается; важные факты поднимаются выше в recall.
1. **Явное забывание** — инструмент `memory_forget` + CLI, soft-delete по сигнатуре с семантической дедупликацией, чтобы тот же факт не вернулся обратно.
...
1. **Pull requests** — fork, branch, commit, push, открываете PR против `main`.
